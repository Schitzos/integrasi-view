<table class="table table-bordered table-hover" id="tabelsetelektronik">
    <thead>
        <tr>
            ...
            <tr>
                ...
                <td style="text-align: center;">
                    @if($p->status_outline==1)
                        <input id="Status_Elektronikal_{{$i}}" name="Status_Elektronikal_{{$i}}" type="checkbox" checked class="make-switch" data-size="small" data-label-icon="fa fa-youtube" data-on-color="success" data-off-color="danger" data-on-text="Disetujui" data-off-text="Ditolak" onchange="ubahsetelektronikal({{$i}},{{$p->id_outline}});">
                    @else
                        <input id="Status_Elektronikal_{{$i}}" name="Status_Elektronikal_{{$i}}" type="checkbox" class="make-switch" data-size="small" data-label-icon="fa fa-youtube" data-on-color="success" data-off-color="danger" data-on-text="Disetujui" data-off-text="Ditolak" onchange="ubahsetelektronikal({{$i}},{{$p->id_outline}});">
                    @endif
                </td>
                <td style="text-align:left;">
                <input type="text" class="form-control"  id="Tanggal_Elektronikal_{{$i}}" name="Tanggal_Elektronikal_{{$i}}" value="{{date('d/m/Y',strtotime($p->tgl_outline))}}" onkeypress="deteksisetelektronik(event,{{$i}},{{$p->id_outline}});">
                </td>
                <td style="text-align:left;">
