...
<td style="text-align:left;">Peraturan Daerah Nomor 11 Tahun {{date('Y',strtotime($data->tgl_cetak))-1}} tentang Anggaran Pendapatan dan Belanja Daerah Tahun Anggaran {{date('Y',strtotime($data->tgl_cetak))}}.</td>
</tr>
<tr>
...
<td style="text-align:justify;vertical-align:top;">Melaksanakan Perjalanan Dinas Dalam Daerah dalam rangka {{$data->tugas}} untuk kegiatan {{$data->nama_kegiatan}} ke {{$data->tujuan}} tanggal 
...
<td style="text-align:left;"  colspan="2">{{$data->pangkat}}/{{$data->nama_golongan}}<br/>@if($data->id_jabatan == 4 || $data->id_jabatan == 5) {{ $data->nama_jabatan }} SEKSI {{ $data->nama_seksi }}@elseif($data->id_jabatan == 3){{ $data->nama_jabatan }} {{ $data->nama_seksi }}@elseif($data->id_bidang == 1) SEKRETARIS DINAS @else {{ $data->nama_jabatan }} {{ $data->nama_bidang }} @endif<br/>-</td>
</tr>
<tr>
...
<td style="text-align:left;vertical-align:top;">Belanja Perjalanan Dinas Dalam Daerah Kegiatan {{$data->nama_kegiatan}} menurut Surat Tugas Perjalanan Dinas Tanggal
@if(date("m",strtotime( $data->tgl_berangkat))==1)
{{ date('d',strtotime($data->tgl_berangkat)) }} Januari {{ date('Y',strtotime($data->tgl_berangkat)) }}
@elseif(date('m',strtotime($data->tgl_berangkat))==2)
{{ date('d',strtotime($data->tgl_berangkat)) }} Februari {{ date('Y',strtotime($data->tgl_berangkat)) }}
